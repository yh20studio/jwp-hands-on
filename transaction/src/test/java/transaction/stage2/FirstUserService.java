...
public class FirstUserService {
    ...
    @Transactional(propagation = Propagation.REQUIRED)
    public Set<String> saveFirstTransactionWithRequiredThrow() {
        final var firstTransactionName = TransactionSynchronizationManager.getCurrentTransactionName();
        userRepository.save(User.createTest());
        logActualTransactionActive();

        String secondTransactionName = null;
        try {
            secondTransactionName = secondUserService.saveSecondTransactionWithRequiredThrow();
        } catch (Exception e) {
